    ...
    for provider in providers:
        try:
            response = await g4f.ChatCompletion.create_async(
                model="gpt-3.5-turbo",
                provider=provider,
                messages=[{"role": "user", "content": f"Переведи на русский и сделай подробный конспект (тезисы, решения): {text}"}],
            )
            if response and len(str(response)) > 20:
                return response
        except:
            continue
    return "⚠️ (Ошибка блока ИИ: сервер перегружен)"

@dp.message(Command("start"))
async def start(m: types.Message):
    await m.answer("🌍 Бот-переводчик (EN/AR -> RU) готов! Пришли ссылку на видео.")
...
async def process(m: types.Message):
    ...
    try:
        ...
        if not os.path.exists(audio_file) and os.path.exists(vid): os.rename(vid, audio_file)
        ...
        for i in range(0, len(audio), 300000):
            ...
            with sr.AudioFile(chunk_p) as src:
                ...
                try:
                    # Пробуем английский, потом арабский
            ...
            if (i // 300000) % 3 == 0: # Обновляем статус реже, чтобы не спамить
                await msg.edit_text(f"⏳ Обработано {i//60000} мин...")

        await msg.edit_text("⏳ 3/3: ИИ переводит и делает конспект...")
        
        # Суммаризация блоками (по 3 фрагмента текста за раз ~ 15 мин видео)
        final_results = []
        ...
        if len(report) > 4000:
            ...
            await m.answer_document(types.FSInputFile("report.txt"), caption="✅ Конспект готов (в файле)")
        else:
